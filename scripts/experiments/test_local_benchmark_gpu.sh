...
name=test-local-benchmark-gpu

# config file
config_file=tabzilla_experiment_config_test_missing.yml
...
for i in ${!MODELS_ENVS[@]}; do
  for j in ${!DATASETS[@]}; do
    ...
    if grep -Fxq "${dataset_name},${model},${experiment_name}" ${result_log}; then
    ...
    (
      ...
      python tabzilla_experiment.py \
        ...
        --output_dir ./results/test/${dataset_name}/${model} \
      ...
      if [ $? -eq 0 ]; then
        ...
        echo "${dataset_name},${model},${experiment_name}" >> ${result_log}
      fi
    ...
    echo "launched experiment for ${model} on ${dataset_name}. (job number ${num_experiments})"
    sleep 1
